...
int main(int argc, char* argv[])
{
	...
	while(1){
		...
		if(defender_selected == 0) // if defender not selected, select role
			role[clnt_no] = rand()%2;
		if(clnt_no == 3 && defender_selected == 0){ // if defender not selected, and receive last client, force to set defender
			role[clnt_no] = 1;
			defender_selected = 1;
		}else if(role[clnt_no] == 1){ // if defender selected, check this
			defender_selected = 1;
		}
...
void *clnt_connection(void *arg)
{
	...
	while((str_len = read(clnt_sock_r, msg, 16)) != 0){ // if socket get message, send this to other socket
...
void send_msg(char *msg, int len)
{
	...
	for(i = 0 ; i < clnt_no ; i++){
		if(matched == 0 && clnt_no <= 4){ // if client number less than 4, send client this form("<Client No.><Role>")
		...
		printf("%d %s\n",i, msg);
		if(i == 3) //  if client number is 4, check matched
			matched = 2;
		if(clnt_no-1 == i && matched > 0){ // if match started, initialize matched variable
			matched --;
		}
	}
	pthread_mutex_unlock(&mutx);
