...
int main(int argc, char* argv[])
{
	...
	serv_addr_r.sin_port = htons(6002);
	...
	while(1){
		...
		for(int i = 0 ; i < 4 ; i++){
			if(clnt_check[i] && buffer[0] != i+48){ // send buffer to other clients except myself
				sendto(serv_sock_r, buffer, BUF_SIZE, 0, (struct sockaddr *)&clnts[i], sizeof(struct sockaddr));
